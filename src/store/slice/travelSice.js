...
export const getAllTravel = createAsyncThunk("/getAllTravel", async () => {
...
export const getAllTravelByCategoryId = createAsyncThunk(
  ...
  async (categoryId) => {
    const response = await publicRequest.get(
      `/travel/getAllTravelByCategoryId/${categoryId}`
    );
    return response.data?.data;
  }
);
...
const travelSlice = createSlice({
  ...
  reducers: {
    ...
    travelBySearch: (state, action) => {
      ...
      state.travelsAfterFilter = state.travels.filter((item) =>
        item.travelName.toLowerCase().includes(action.payload.toLowerCase())
      );
    },
    travelFiltersByCategoryId: (state, action) => {
      if (action.payload === 0) {
        state.travelsAfterFilterByCategoryId = state.travelsByCategoryId.sort(
          (x, y) => {
            return parseInt(x.id) - parseInt(y.id);
          }
        );
