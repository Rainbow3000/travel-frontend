...
import {getAllTravelByCategoryId,travelFilters,travelFiltersByCategoryId} from "../../store/slice/travelSice";
...
const CategoryDetails = () => {
  ...
  const {travelsByCategoryId,travelsAfterFilterByCategoryId} = useSelector(state=>state.travel)
          ...
          <div className="category-travel-list">
                <div className="category-sort">
                    <ul>
                        <h2>Sắp xếp:</h2>
                        {
                            ["Đề xuất","Giá:Thấp đến cao","Giá:Cao đến thấp"].map((item,index)=>{
                                return <li onClick={()=>handleChangeCateActive(index)} style={{
                                    color:cateActive === index && "blue",
                                    border:cateActive === index && "2px solid blue",
                                    backgroundColor:cateActive !== index ? "rgba(128, 128, 128, 0.203)" :"white" 
                ...
                <div className="travel-list">
                  {
                    travelsAfterFilterByCategoryId && travelsAfterFilterByCategoryId.map((item)=>{
                      return(
                        <TravelCate item={item}/>
                      )
                    })
                  }
